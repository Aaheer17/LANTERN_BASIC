...
class GenerativeModel_COPY(nn.Module):
    """
    ...
    def train_one_epoch(self):
        ...
        for batch_id, x in enumerate(self.train_loader):
            #print(f'now processing batch no {batch_id}')
            ...
            if self.epoch in self.attn_epoch and batch_id==0:
                if hasattr(self.net, 'plot_attention_rollout'):
                        self.net.plot_attention_rollout(save_path=self.doc.get_file(f'attn_epoch{self.epoch}_batch{batch_id}.png'))
